...
function readInput(): Input {
  ...
  return { readmeFileName, type, jacocoFileName, minimCoverage }
}
...
export async function run(): Promise<void> {
  try {
    ...
    if (!fileFound) {
      ...
      core.warning(
        `#run file not found : ${input.readmeFileName} using ${_defaultReadmeName}`
      )
    }
    ...
    const oldCoverageSource = fileUtils.findInFile(
      input.readmeFileName || _defaultReadmeName,
    ...
    if (isSupported(oldCoverageSource)) {
      ...
      core.info(
        `#run handle supported oldCoverageValue type ${oldCoverageValue}`
      )
      ...
      function jacocoCoverage(inputCoverage: string): Record<string, number> {
        const result: Record<string, number> = {}
        const keyValuePairs = inputCoverage.split(' ')
      ...
      const jacocoNewCoverage: Record<string, number> =
        jacocoCoverage(currentBuildCoverage)
      core.info(
        `#run new jacocoNewCoverage :  ${jacocoNewCoverage.missed}: ${jacocoNewCoverage.covered}`
      )
      const latestTotal: number =
        jacocoNewCoverage.missed + jacocoNewCoverage.covered

      // const latestCoverage: string = ((jacocoNewCoverage.covered / latestTotal)*100).toPrecision(4);
      const latestCoverageRatio = parseFloat(
        (jacocoNewCoverage.covered / latestTotal).toFixed(4)
      )
      ...
      let badgeColor = defaultCoverageColor.red
      if (latestCoverageRatio > parseFloat(_defaultGreenMinim)) {
        core.info(
          `#run green ${latestCoverageRatio} > ${parseFloat(
            _defaultGreenMinim
          )}`
        )
      ...
      const enableCovReplace = false
      if (enableCovReplace) {
        core.info(
          `#run readmeFileName = ${input.readmeFileName}  oldCoverage = ${oldCoverageSource} latestCoverage = ${latestCoverage}%`
        )
        fileUtils.replace(
          oldCoverageSource,
          oldCoverageValue,
